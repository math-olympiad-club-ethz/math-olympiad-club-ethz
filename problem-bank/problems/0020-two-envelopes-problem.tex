...
\]
which means we keep the first envelope if the generated number \( U\left(\xi\right) \) satisfies \(0\leq  U\left(\xi\right) \leq g\left(\pi_1\left(w\right)\right) \), and we switch otherwise. For example, if $g=\text{cte}_0$ then we always switch, while if $g=\text{cte}_1$ then we always keep. 
\\\\
...
\]
Since \(g'\) is bounded above by \(1\) and bounded below by $0$, this implies $nc< 1$ for every \(n\), which is impossible since $c$ is not an \href{https://en.wikipedia.org/wiki/Archimedean_property}{infinitesimal element} w.r.t. $1$. However if we had more information on $\alpha$ (say some lower or/and upper bound) we may tune hyperparameter $t,c\in\mathbb{R}_{>0}$ by doing first order optimization on for examples:
\\
...
for label, g_func in g_functions.items():
    ...
    print(f"  Improvement over naive strategy: {improvement:+.6f}\n")
\end{lstlisting}
