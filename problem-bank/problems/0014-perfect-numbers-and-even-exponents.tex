...
so \(p_{1} + 1\) divides \(2n\). Due to \(p_{1} + 1 > 2\), at least one of the primes \(p_{1}, \ldots, p_{k}\) divides \(p_{1} + 1\). The primes \(p_{3}, \ldots, p_{k}\) are greater than \(p_{1} + 1\) and \(p_{1}\) cannot divide \(p_{1} + 1\) (else $p_{1}$ would divide $1$), so \(p_{2}\) must divide \(p_{1} + 1\) i.e. $\exists t\in\mathbb{N}^{*}$ with $p_{1}+1=tp_{2}$. Since \(tp_{2}=p_{1} + 1 < 2p_{1}<2p_{2}\), this is possible only if $t=1$ i.e. \(p_{2} = p_{1} + 1\), therefore \(p_{1} = 2\) and \(p_{2} = 3\) (the only two consecutive primes are $2$ and $3$). Hence, \(6 \mid n\).
\\\\
