...
which is equivalent to 
\[
p\left( mn - 2m - 2n - 4 \right) = 2\left( m+2 \right)\left( n+2 \right).
\]
...
Since \(10 \equiv 1 \pmod{3}\), we have \(10^n \equiv 1 \pmod{3}\), so \(10^n + 2 \equiv 0 \pmod{3}\). Hence \(\frac{10^n + 2}{3}\) is an integer. Therefore, \(A + 2B + 4\) is a perfect square.
\subsection{AIME (2000)} 
